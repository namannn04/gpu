...
const GPUResults = ({
  ...
  const sortedResults = useMemo(() => {
    return [...results].sort((a, b) => {
      ...
      else return b.price_per_hour - a.price_per_hour;
    });
  }, [results, sortOrder]);

  const minPrice = Math.min(...results.map((g) => g.price_per_hour));
    ...
    <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 p-6 shadow-2xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <select
          ...
          className="bg-gray-700 text-white text-sm px-3 py-2 rounded-md border border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      {results.length > 0 ? (
        <div className="space-y-4">
          ...
          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2">
            {sortedResults.map((gpu, idx) => (
              <GPUCard
                key={idx}
                gpu={gpu}
                isBestMatch={gpu.price_per_hour === minPrice}
              />
            ))}

            {/* Fallback NVIDIA A100 card */}
